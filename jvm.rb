...
class JVM
	...
	def resolve_field jvmclass, field
		if jvmclass.resolved.key? field
			...
			resolve_field(load_class(jvmclass.class_file.get_attrib_name(jvmclass.class_file.super_class)), field)
		else
	...
	def resolve_special_method reference_jvmclass, method_jvmclass, method
		if	reference_jvmclass.class_file.access_flags.super? &&
			...
			type_equal_or_superclass?(reference_jvmclass.class_file.this_class_type, method_jvmclass.class_file.this_class_type)
	...
	def resolve_method jvmclass, method
		if jvmclass.resolved.key? method
			...
			resolve_method(load_class(jvmclass.class_file.get_attrib_name(jvmclass.class_file.super_class)), method)
		else
	...
	def java_to_native_string reference
		...
		arrayref = run_and_return Frame.new(
				resolve_method(load_class(reference.class_type), method),
				method,
				[reference]
		)
	...
	def resolve_exception_handler frame, exception
		frame.exceptions.each do |e|
			if frame.pc - 1 >= e.start_pc && frame.pc - 1 < e.end_pc &&
				(e.catch_type.zero? ||
				type_equal_or_superclass?(exception.class_type,
	...
	def type_equal_or_superclass?(class_type_a, class_type_b)
		if class_type_a == class_type_b
			true
		elsif class_type_a.chr == '['
			if class_type_b.chr == '['
				is_type_equal_or_superclass(class_type_a[1..-1], class_type_b[1..-1])
			else
				class_type_b == 'java/lang/Object'
			end
		else
			jvmclass = load_class class_type_a
			return true if	jvmclass.class_file.super_class.nonzero? &&
							type_equal_or_superclass?(
								jvmclass.class_file.get_attrib_name(jvmclass.class_file.super_class),
								class_type_b
							)
			jvmclass.class_file.interfaces.each.any? do |i|
				return true if type_equal_or_superclass?(jvmclass.class_file.get_attrib_name(i), class_type_b)
			end
			false
		end
	end
			...
			while frame.pc < frame.code_attr.code.length
				begin
					...
					when 192
						...
						if reference && !type_equal_or_superclass?(reference.class_type, frame.jvmclass.class_file.get_attrib_name(class_index))
					...
					when 193
						...
						reference = frame.stack.pop
						if reference
							frame.stack.push(type_equal_or_superclass?(reference.class_type,
					...
					when 197
						...
						frame.stack.push JavaInstanceArray.new(
